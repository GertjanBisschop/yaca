...
def single_step_prob(pairwise_overlap, pairs, last_event, t, rho, node_times):
    ...
    return single_event * np.exp(-mean_intensity)
...
def get_p(ts, rho):    
    ...
    t = 0
    last_event = 0
    pairwise_overlap = {pair:sequence_length for pair in itertools.combinations(range(num_samples), 2)}
    ...
    for i in range(num_samples, num_nodes):
        ...
        p *= single_step_prob(pairwise_overlap, pairs, last_event, t, rho, ts.nodes_time)
